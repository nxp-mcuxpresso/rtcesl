...
extern inline frac32_t MLIB_Msu4RndSat_F32_FAsmi(register frac32_t f32MinMult1, register frac32_t f32MinMult2, register frac32_t f32SubMult1, register frac32_t f32SubMult2)
{
	...
	asm(mpy64 f32MinMult1,f32MinMult2,f32MinMult2);	/* f32Val1:Y = f32MinMult1 * f32MinMult2 */
	
	asm(tfr	Y,f32MinMult1);						    /* f32MinMult1 = Y */
	
	asm(neg f32SubMult2);
	
	asm(sat f32SubMult2);							/* Saturation */

	asm(mpy64 f32SubMult1,f32SubMult2,f32SubMult2);	/* f32Val2:Y = f32SubMult1 * f32SubMult2 */
	
	asm(add f32MinMult2,f32SubMult2);				/* f32Val1 = f32Val1 + f32Val2 */
	
	asm(add	f32MinMult1,Y);						    /* Y = Y + f32MinMult1 */
	...
	asm(adc	Y,f32SubMult2);							/* f32Val1 = f32Val1 + Rounding Bit + Carry Bit */
